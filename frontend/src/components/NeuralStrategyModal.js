        ...
        <AnimatePresence>
            <div className="neural-modal-overlay">
                ...
                >
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '3rem' }}>
                            ...
                            <div style={{ height: '440px' }}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={analytics?.dailyData.slice(viewMode === 'today' ? 29 : viewMode === 'week' ? 23 : 0)}>
                                        ...
                                        <XAxis dataKey="label" stroke="rgba(255,255,255,0.2)" fontSize={11} fontWeight={900} axisLine={false} tickLine={false} dy={15} />
                                        <YAxis stroke="rgba(255,255,255,0.2)" fontSize={11} fontWeight={900} axisLine={false} tickLine={false} />
                                        <Tooltip
                                            cursor={{ fill: 'rgba(255,255,255,0.03)' }}
                                            contentStyle={{ background: '#0a0f19', border: '1px solid #00f2ff', borderRadius: '24px', padding: '15px' }}
                                            itemStyle={{ color: '#00f2ff', fontWeight: 1000 }}
                                        />
                                        <Bar
                                            dataKey="sales"
                                            fill="url(#barGlow)"
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '2rem' }}>
                                ...
                                <motion.div whileHover={{ y: -15, scale: 1.02 }} className="bento-item-apex" style={{ gridColumn: 'span 2', background: 'linear-gradient(135deg, rgba(255,188,0,0.1), rgba(0,0,0,0.2))' }}>
                                ...
                                <motion.div whileHover={{ y: -15, scale: 1.02 }} className="bento-item-apex" style={{ background: 'rgba(188, 0, 255, 0.05)' }}>
                                    ...
                                    <p style={{ margin: '10px 0 0', fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', fontWeight: 800 }}>LIVE_THROUGHPUT</p>
                                </motion.div>

                                <motion.div whileHover={{ y: -15, scale: 1.02 }} className="bento-item-apex" style={{ background: 'rgba(0, 242, 255, 0.05)' }}>
                                    <div className="apex-label">Weekly Sales</div>
                                    <div className="apex-value">{analytics?.sold7d} <small style={{ fontSize: '1rem', opacity: 0.4 }}>items</small></div>
                                    <p style={{ margin: '10px 0 0', fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', fontWeight: 800 }}>7-DAY_TOTAL</p>
                                </motion.div>
                            ...
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bento-item-apex" style={{ padding: '2rem', border: '1px solid rgba(16, 185, 129, 0.3)', background: 'rgba(16, 185, 129, 0.05)' }}>
                                <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                                    ...
                                    <div>
                                        <h4 style={{ margin: 0, fontSize: '1.2rem', color: '#fff' }}>Recommended Action</h4>
                                        <p style={{ margin: '5px 0 0', fontSize: '1rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.4 }}>Demand is shifting towards weekend peaks. Suggest adjusting pricing node by <strong>+2.5%</strong>.</p>
                                    </div>
                                </div>
                            </motion.div>
                        </div>
                    ...
                    <AnimatePresence>
                        {showDeepInsights && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} style={{ marginTop: '4rem', padding: '3rem', background: 'rgba(0, 242, 255, 0.03)', borderRadius: '40px', border: '1px solid rgba(0, 242, 255, 0.2)', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '3rem' }}>
                                <div className="insight-node">
                                    <h5 className="apex-label" style={{ color: '#00f2ff' }}>Buying Pattern</h5>
                                    <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1rem', lineHeight: 1.6 }}>Customers buying <strong>{product}</strong> also peak in high-value beverage nodes.</p>
                                </div>
                                <div className="insight-node">
                                    <h5 className="apex-label" style={{ color: '#bc00ff' }}>Neural Forecast</h5>
                                    <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1rem', lineHeight: 1.6 }}>Projected growth of <strong>+18.4%</strong> in next supply cycle detected.</p>
                                </div>
                                <div className="insight-node">
                                    <h5 className="apex-label" style={{ color: '#ffbc00' }}>Smart Restock</h5>
                                    <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1rem', lineHeight: 1.6 }}>Acquisition latency minimized if restocked within <strong>48 hours</strong>.</p>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
